<!DOCTYPE html>
	<head>
		<title>LTE Elections | Members</title>
		<?php require(__DIR__ . '/metadata.html'); ?>
	</head>
	<body>
		<?php require(__DIR__ . '/navbar.html'); ?>
		
		<div class="article">
			<div class="section"><h2>Meet the members of LTE!</h2></div>
			<a href="thottie_hollie.php"><div class='section newssection'>
				<h3>Thottie Hollie</h3>
				<p>An intelligent thot</p>
				</div></a>
			<a href="uncle_alex.php"><div class='section newssection'>
				<h3>Uncle Alex</h3>
				<p>The uncle himself</p>
				</div></a>
			<a href="romulus.php"><div class='section newssection'>
				<h3>Romulus</h3>
				<p>The big bad Rom!</p>
				</div></a>
			<a href="orden.php"><div class='section newssection'>
				<h3>Orden</h3>
				<p>Returns from Hogwarts</p>
				</div></a>
			<a href="hoop28.php"><div class='section newssection'>
				<h3>Hoop28</h3>
				<p>Who ya gonna hoop?</p>
				</div></a>
			<a href="computerguy15.php"><div class='section newssection'>
				<h3>Computerguy15</h3>
				<p>Does not computer</p>
				</div></a>
			<a href="chancellor_federalist.php"><div class='section newssection'>
				<h3>Chancellor Federalist</h3>
				<p>All hail the great Federal Chancellor</p>
				</div></a>
			<a href="alligator.php"><div class='section newssection'>
				<h3>Alligator</h3>
				<p>The libertarian socialist</p>
				</div></a>
			<a href="adam.php"><div class='section newssection'>
				<h3>Adam</h3>
				<p>e</p>
				</div></a>
		</div>
	</body>
</html>
